...
from src.utils.db import add_user, get_user, get_user_info, DB_PATH
...
async def resend_lesson(callback: CallbackQuery):
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute('''
            SELECT course_id, tier, current_lesson
            FROM user_courses
            WHERE user_id = ?
    ...
    course_id, tier, lesson = course_data
    ...
    for material in materials:
        if material['type'] == 'text':
            ...
            async with aiofiles.open(material['file_path'], 'rb') as photo:
                await callback.message.answer_photo(photo)
        elif material['type'] == 'video':
            async with aiofiles.open(material['file_path'], 'rb') as video:
...
async def process_activation(message: Message, state: FSMContext):
    success, course_id, tier = await verify_course_code(message.text, message.from_user.id)
    if success:
        with open('data/courses.json', 'r', encoding='utf-8') as f:
            courses = json.load(f)
        course = courses[course_id]
        tier_data = course['tiers'][tier]
        
        await message.answer(
            f"✅ Активирован курс '{course['name']}'\n"
            f"Тариф: {tier_data['name']}\n\n"
